...
from ViT_custom_local544444_256_rp_noise import Generator
import numpy as np
...
def setup_dataflow(self, dataset, train_idx, val_idx):
...
def main():
    ...
    seq_dataset = DecompTraceDataset(
        root_dir=os.path.join(dataset_config.root_dir),
        target_length=dataset_config.target_length,
        hflip=dataset_config.hflip,
        vflip=dataset_config.vflip,
        flip_rate=dataset_config.flip_rate,
        randshift=dataset_config.randshift,
        seed=dataset_config.seed,
        sep_tok=dataset_config.sep_tok,
        normalize=dataset_config.normalize,
        num_classes=dataset_config.num_classes,
        flat=dataset_config.flat
    )

    dataset_y = np.array(seq_dataset.all_labels)
    train_y, test_y = train_test_split(
        dataset_y, train_size=0.85, random_state=42)
        ...
        if epoch < opt.G_epochs:
            for _, x_batch, _, y_batch, _ in train_loader:
                gen_y = torch.from_numpy(np.int32(np.random.choice(
                    opt.num_classes, (y_batch.shape[0],)))).long()
                x_batch = torch.from_numpy(x_batch)
                y_batch = torch.from_numpy(y_batch).long()
                d_loss, train_acc = T.train_batch_disc(
                    x_batch, y_batch, gen_y, weight_gen_loss)
                D_loss += d_loss
                Train_acc += train_acc
                for j in range(2):
                    gen_y_ = y_batch
                    G_loss += T.train_batch_gen(x_batch,
                                                gen_y_, weight_gen_loss)
        else:
            # train Classifier
            for _, x_batch, _, y_batch, _ in train_loader:
                index += 1
    ...
    # test
        ...
        if (epoch+1) % (opt.fre_print) == 0:
            print("Iteration %d, D_loss = %.4f,G_loss = %.4f,train acc = %.4f, test acc = %.4f,best acc = %.4f,lr = %.8f" % (
